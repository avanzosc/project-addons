...
class ProjectTack(models.Model):
    ...
    calendar_event_ids = fields.One2many(
        string="Meetings", comodel_name="calendar.event", inverse_name="task_id"
    )
    ...
    def action_to_calendar_event(self):
        ...
        if self.user_id:
            context.update({"default_user_id": self.user_id.id})
        if self.partner_id:
            context.update({"default_partner_ids": [(6, 0, self.partner_id.ids)]})
        return {
            "name": _("Meetings"),
