...
class ProjectProject(models.Model):
    ...
    tasks_planned_hours = fields.Float(
        string="Initially Tasks Planned Hours",
        tracking=True,
        store=True,
        copy=False,
        compute="_compute_tasks_hours_info",
    )
    tasks_real_hours = fields.Float(
        string="Tasks Real Hours",
        tracking=True,
        store=True,
        copy=False,
        compute="_compute_tasks_hours_info",
    ...
    milestones_percentages = fields.Float(
        string="Total milestones percentages",
        tracking=True,
        store=True,
        copy=False,
        compute="_compute_milestones_percentages",
    )
    milestones_percentages_solved = fields.Float(
        string="Total milestones percentages solved",
        tracking=True,
        store=True,
        copy=False,
        compute="_compute_milestones_percentages",
    ...
    def _compute_tasks_hours_info(self):
        for project in self:
            ...
            if project.all_task_ids:
                tasks_planned_hours = sum(project.all_task_ids.mapped("planned_hours"))
    ...
    @api.depends(
        "milestone_ids", "milestone_ids.quantity_percentage", "milestone_ids.is_reached"
    )
        ...
        for project in self:
            ...
            if project.milestone_ids:
                ...
                if milestones_reached:
                    ...
                    milestones_percentages_solved = milestones_percentages_solved * 100
            if milestones_percentages > 0:
                if milestones_percentages_solved > milestones_percentages:
                    ...
                    milestones_progress = round(
                        100.0 * milestones_percentages_solved / milestones_percentages,
                        2,
                    )
            project.milestones_percentages = milestones_percentages
            project.milestones_percentages_solved = milestones_percentages_solved
